  ...
  const [savedNews, setSavedNews] = useState([]);
    ...
    const filteredItems = remainingItems.filter(
        (item) =>
            ...
            item.published_at?.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <Stack direction="horizontal" className="w-100">
                <Row className="g-3 g-sm-4 g-md-4 g-lg-4 g-xl-4 pt-5 p-sm-4 p-md-5 p-lg-5 p-xl-5">
                    {filteredItems.map((item, index) => (
                        <Col key={index} xs={12} sm={6} md={4} lg={4} xl={3}  className="mb-3">
...
export default AllTechCrunchData;